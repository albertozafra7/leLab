...
@app.get("/inference-camera-feed/{cam_key}")
def inference_camera_feed(cam_key: str):
    """MJPEG stream for one inference preview camera.

    Streams while inference is active and the named camera exists; ends
    cleanly when inference stops so the browser <img> finishes naturally.
    """
    from fastapi.responses import StreamingResponse
    return StreamingResponse(
        inference_camera_feed_frames(cam_key),
...
def rename_job(job_id: str, body: RenameJobRequest):
    try:
        return job_registry.rename(job_id, body.name)
    except JobNotFoundError as exc:
        raise HTTPException(status_code=404, detail=f"Job {job_id!r} not found") from exc
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
